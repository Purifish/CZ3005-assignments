...
def rotate_orientation(orientation, number_of_90_degree_clockwise_rotations):
    ...
    new_index = (current_index +
                 number_of_90_degree_clockwise_rotations) % len(orientations)
...
class World:
    def __init__(self, agent_original_position=(1, 1), agent_original_orientation='north', wumpus_positions=[], coin_positions=[], portal_positions=[], wall_positions=[], world_size=(7, 6)):
        ...
        self.db.consult('AlphaBlur-Agent.pl')

        self.starting_arguments = locals()
        # Remove unrequired arguments
        self.starting_arguments.pop("self", None)
    ...
    def initialise_world(self, agent_original_position, agent_original_orientation, wumpus_positions, coin_positions, portal_positions, wall_positions):
        ...
        self.agent = Agent(
            self.agent_original_position[0], self.agent_original_position[1], self.agent_original_orientation)
        ...
        self.world[self.agent.position[1]
                   ][self.agent.position[0]].occupant = self.agent
        self.world[self.agent.position[1]
                   ][self.agent.position[0]].is_visited = True
        self.world[self.agent.position[1]
                   ][self.agent.position[0]].is_safe = True
        self.world[self.agent.position[1]
                   ][self.agent.position[0]].senses['confounded'] = True
        current_position = self.agent.position

        if current_position in self.coin:
            self.world[self.agent.position[1]
                       ][self.agent.position[0]].senses['glitter'] = True
        ...
        for x, y in relative_neighbors:
            neighbor_x, neighbor_y = self.agent.position[0] + \
                x, self.agent.position[1] + y
            ...
            if neighbor_coordinates in self.wumpus:
                self.world[self.agent.position[1]
                           ][self.agent.position[0]].senses['stench'] = True
            if neighbor_coordinates in self.portals:
                self.world[self.agent.position[1]
                           ][self.agent.position[0]].senses['tingle'] = True
        agent_relative_position = list(self.db.query("current(X, Y, Z)"))[0]
        current_node = self.world[self.agent.position[1]
                                  ][self.agent.position[0]]

        if 'stench' in current_node.senses:
            self.db.assertz(
                f'stench({agent_relative_position["X"]},{agent_relative_position["Y"]})')

        if 'tingle' in current_node.senses:
            self.db.assertz(
                f'tingle({agent_relative_position["X"]},{agent_relative_position["Y"]})')

        if 'glitter' in current_node.senses:
            self.db.assertz(
                f'glitter({agent_relative_position["X"]},{agent_relative_position["Y"]})')

        if 'bump' in current_node.senses:
            self.db.assertz(
                f'bump({agent_relative_position["X"]},{agent_relative_position["Y"]})')

        if 'scream' in current_node.senses:
            self.db.assertz(
                f'scream({agent_relative_position["X"]},{agent_relative_position["Y"]})')

        list(self.db.query(
            f"reposition({self.world[self.agent.position[1]][self.agent.position[0]].stringify_senses()})"))
    ...
    def update_agent(self, action):
        ...
        if action == 'pickup':
            ...
            self.world[self.agent.position[1]
                       ][self.agent.position[0]].senses.pop('glitter', None)
            updated_senses = self.world[self.agent.position[1]
                                        ][self.agent.position[0]].stringify_senses()
        ...
        if action == 'shoot':
            ...
            coordinate_to_check = (
                self.agent.position[0] + to_check[0], self.agent.position[1] + to_check[1])
            while coordinate_to_check[0] >= 0 and coordinate_to_check[0] <= self.size[1] - 1 and coordinate_to_check[1] >= 0 and coordinate_to_check[1] <= self.size[0] - 1:
                if coordinate_to_check in self.wumpus:  # If coordinate is a wumpus
                    self.world[self.agent.position[1]][self.agent.position[0]
                                                       ].senses['scream'] = True  # Scream
                    # Remove wumpus from node
                    self.world[coordinate_to_check[1]
                               ][coordinate_to_check[0]].occupant = None
                    # Remove wumpus from node
                    self.wumpus.remove(coordinate_to_check)

                    to_remove_stench = [(coordinate_to_check[0] - 1, coordinate_to_check[1]), (coordinate_to_check[0] + 1, coordinate_to_check[1]),
                                        (coordinate_to_check[0], coordinate_to_check[1] - 1), (coordinate_to_check[0], coordinate_to_check[1] + 1)]
                    for x, y in to_remove_stench:
                        self.world[y][x].senses.pop('stench', None)
                    break

                coordinate_to_check = (
                    coordinate_to_check[0] + to_check[0], coordinate_to_check[1] + to_check[1])
        ...
        if action == 'moveforward':
            if self.agent.orientation == 'north':
                ...
                new_x, new_y = self.agent.position[0] + \
                    1, self.agent.position[1]
            else:
                new_x, new_y = self.agent.position[0] - \
                    1, self.agent.position[1]
        ...
        if (new_x, new_y) in self.walls:
            ...
            self.world[self.agent.position[1]
                       ][self.agent.position[0]].senses['bump'] = True
            updated_senses = self.world[self.agent.position[1]
                                        ][self.agent.position[0]].stringify_senses()
            list(self.db.query(
                f"move({action}, {updated_senses})"))
            return
        ...
        if (new_x, new_y) in self.portals:
            ...
            self.agent_original_position = (new_x, new_y)
            ...
            # Update agent
            list(self.db.query(
                f"reposition({self.world[new_y][new_x].stringify_senses()})"))
            return
        ...
        if 'bump' in self.world[self.agent.position[1]][self.agent.position[0]].senses:
            self.world[self.agent.position[1]
                       ][self.agent.position[0]].senses.pop('bump', None)
        ...
        if 'scream' in self.world[self.agent.position[1]][self.agent.position[0]].senses:
            self.world[self.agent.position[1]
                       ][self.agent.position[0]].senses.pop('scream', None)

        self.world[self.agent.position[1]][self.agent.position[0]
                                           ].occupant = None  # Remove agent from previous location
        ...
        # Add this location to visited
        self.agent_visited_locations.add((new_x, new_y))
        self.world[self.agent.position[1]][self.agent.position[0]
                                           ].occupant = self.agent  # Add agent to new location
        self.world[self.agent.position[1]][self.agent.position[0]
                                           ].is_visited = True  # Set new location as visited
        self.world[self.agent.position[1]][self.agent.position[0]
                                           ].is_safe = True  # New location is safe
    ...
    def perform_agent_action(self, action):
        ...
        """
        current_senses = self.world[self.agent.position[1]
                                    ][self.agent.position[0]].stringify_senses()
    ...
    def display_initial_world(self):
        ...
        for y in range(self.size[1]):
            ...
            for x in range(self.size[0]):
                ...
                if (x, y) == self.agent_original_position:
                    n.occupant = Agent(
                        *self.agent_original_position, self.agent_original_orientation)
        ...
        for row in world:
            for i in range(3):
                for node in row:
                    print(" ".join(node.get_symbols_to_print()
                          [3*i:3*i+3]), end="  ")
                print()
        print()
    ...
    def display_relative_world(self):
        ...
        relative_safe_locations = set(
            [(location['X'], location['Y']) for location in relative_safe_locations])

        relative_visited_locations = list(self.db.query("visited(X, Y)"))
        relative_visited_locations = set(
            [(location['X'], location['Y']) for location in relative_visited_locations])

        relative_glitter_locations = list(self.db.query("glitter(X, Y)"))
        relative_glitter_locations = set(
            [(location['X'], location['Y']) for location in relative_glitter_locations])

        relative_wumpus_locations = list(self.db.query("wumpus(X, Y)"))
        relative_wumpus_locations = set(
            [(location['X'], location['Y']) for location in relative_wumpus_locations])

        relative_portal_locations = list(self.db.query("confundus(X, Y)"))
        relative_portal_locations = set(
            [(location['X'], location['Y']) for location in relative_portal_locations])

        relative_wall_locations = list(self.db.query("wall(X,Y)"))
        relative_wall_locations = set(
            [(location['X'], location['Y']) for location in relative_wall_locations])

        relative_travelled_coordinates = set(
            [(x - self.agent_original_position[0], y - self.agent_original_position[1]) for x, y in self.agent_visited_locations])
        ...
        for y in range(-max_y + 1, max_y):
            ...
            for x in range(-max_x + 1, max_x):
                ...
                if absolute_x < 0 or absolute_x > self.size[0] - 1 or absolute_y < 0 or absolute_y > self.size[1] - 1:
                    ...
                    new_agent_orientation = rotate_orientation(
                        self.agent.orientation, rotations_required)
                    a = Agent(
                        self.agent.position[0], self.agent.position[1], new_agent_orientation)
                    n = Node()
        ...
        for absolute_y in range(len(world)):
            for absolute_x in range(len(world[absolute_y])):
                relative_coordinates = (
                    absolute_x - floor(len(world[absolute_y]) / 2), floor(len(world) / 2) - absolute_y)
        ...
        print(self.world[self.agent.position[1]]
              [self.agent.position[0]].format_senses_to_print())
        for row in world:
            for i in range(3):
                for node in row:
                    print(" ".join(node.get_symbols_to_print()
...
def testworld1():
    w = World(agent_original_position=(3, 4), agent_original_orientation='east', wumpus_positions=[(6, 4)], coin_positions=[(8, 8), (3, 2)], portal_positions=[
              (5, 3), (1, 7)], wall_positions=[(7, 7), (8, 7), (4, 4), (2, 2), (2, 3), (3, 3), (4, 3), (4, 2), (7, 4)], world_size=(10, 10))
...
def testworld2():
    w = World(portal_positions=[(4, 1)], agent_original_position=(
        1, 1), agent_original_orientation="east")
    w.display_initial_world()
    ...
    for i in range(3):
        ...
        safe_positions = set([(a['X'], a['Y'])
                             for a in list(w.db.query("safe(X,Y)"))])
...
def testworld3():
    w = World(agent_original_position=(3, 4), agent_original_orientation='east', wumpus_positions=[
              (6, 4)], coin_positions=[(8, 8)], portal_positions=[(5, 3), (1, 7), (6, 8)], world_size=(10, 10))
    w.display_initial_world()
    w.display_relative_world()
    for i in range(6):
        w.update_agent("moveforward")
...
if __name__ == "__main__":
    ...
    w = World(agent_original_position=(2, 2), agent_original_orientation='east', wumpus_positions=[(5, 5)], coin_positions=[(5, 6), (5, 8), (1, 2)], portal_positions=[
              (4, 8), (4, 1), (2, 4)], wall_positions=[(4, 4), (4, 5), (4, 6), (4, 7), (5, 7), (6, 7), (6, 6), (6, 5), (6, 4), (8, 7), (7, 5)], world_size=(10, 10))
