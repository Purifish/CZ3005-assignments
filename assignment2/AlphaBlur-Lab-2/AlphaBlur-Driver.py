...
class Driver(object):
    def __init__(self, file_name: str = "agent.pl") -> None:
        # Initalise agent.
    ...
    def start_explore_game(self) -> None:
        ...
        while bool(list(self.agent.prolog.query("explore(L)", maxresult=10))):
            ...
            for move in moves:
                ...
                # Order Agent to pickup if there's coin in the current cell.
                if self.abs_map[self.abs_x][self.abs_y][7] == Map.GLITTER:
                    print("[Driver Found a Coin]", end="")
                    self.command_agent(Moveset.pick_up)
    ...
    def evaluate_agent(self, entered_portal: bool = False) -> bool:
        ...
        # 2.0 Check for Wall
        for soln in self.agent.prolog.query("wall(X,Y)"):
            wall_x = soln['X'] + REL_OFFSET_X
            wall_y = soln['Y'] + REL_OFFSET_Y
            try:
                if self.abs_map[wall_x - rel_x_boundary[0]][wall_y - rel_y_boundary[0]][1] != Map.WALL:
                    inference = False
                    print("[Inference] Wall check failed - ", end="")
                    print("There's no wall at ",
